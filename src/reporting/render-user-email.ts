...
function buildManagerFrictionNote(report: Omit<NormalizedUserReport, "html" | "templateMode">) {
  if (report.content.inferredFrictionTheme.trim().length > 0) {
    return {
      text: report.content.inferredFrictionTheme,
      attr: "AI-inferred from available activity patterns only · not a submitted note.",
    };
  }

  return {
    text: "No inferred friction theme was strong enough to surface from the currently available activity patterns.",
    attr: "AI-inferred from available activity patterns only · not a submitted note.",
  };
}
...
function buildLeaderFrictionTheme(report: Omit<NormalizedUserReport, "html" | "templateMode">) {
  if (report.content.inferredFrictionTheme.trim().length > 0) {
    return {
      text: report.content.inferredFrictionTheme,
      attr: "AI-inferred from available activity patterns only · not a submitted note.",
  ...
  return {
    text: "No inferred friction theme was strong enough to surface across the currently available span activity patterns.",
    attr: "AI-inferred from available activity patterns only · not a submitted note.",
  };
}
...
function renderBusinessOwnerHtml(report: Omit<NormalizedUserReport, "html" | "templateMode">) {
  ...
  const friction = buildManagerFrictionNote(report);
  ...
  const body = `
    ...
    ${sectionWrap(`${sectionLabel("Inferred friction theme")}${quoteBlock(friction.text, friction.attr)}`, false)}
    ${footerBlock([
...
function renderSuperAdminHtml(report: Omit<NormalizedUserReport, "html" | "templateMode">) {
  ...
  const friction = buildLeaderFrictionTheme(report);
